...
auto TomlManifestStore::read(const fs::Path &path) const -> Result<Manifest> {
  fs::StdFileSystem default_fs;
  const fs::FileSystem &fs =
      fs_ ? *fs_ : static_cast<const fs::FileSystem &>(default_fs);
...
auto TomlManifestStore::write(const fs::Path &path, const Manifest &manifest)
    -> Result<void> {
  toml::table table{};
  ...
  if (fs_) {
    return fs_->write_text_file(path, file, fs::FileWriteMode::Overwrite);
  } else {
    fs::StdFileSystem default_fs;
    return default_fs.write_text_file(path, file, fs::FileWriteMode::Overwrite);
  }
}
